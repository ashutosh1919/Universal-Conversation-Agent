...
class Pandorabots:
    ...
    def talk(self, input1, usebotkey=False):
        ...
        if usebotkey:
            ...
            query["user_key"] = self.user_key
        if 'sessionid' in input1:
            query['sessionid'] = input1['sessionid']
        if 'recent' in input1:
            query['recent'] = str(input1['recent'])
        if 'reset' in input1:
            query['reset'] = str(input1['reset'])
        if 'trace' in input1:
            query['trace'] = str(input1['trace'])
        if 'client_name' in input1:
            query['client_name'] = input1['client_name'] 
        if 'that' in input1:
            query['that'] = input1['that']
        if 'topic' in input1:
            query['topic'] = input1['topic']
        if 'reload' in input1:
            query['reload'] = str(input1['reload'])
        if 'extra' in input1:
            query['extra'] = str(input1['extra'])
        if 'message' in input1:
            query['input'] = str(input1['message'])
